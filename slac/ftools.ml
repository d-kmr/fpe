...
exception ExpToBExp
exception NotAVariable of bytes
...
(** Shortcut And *)
let rec s_and f = function
	| [] -> true
	| x::xs -> if f x then s_and f xs else false

(** Shortcut Or *)
let rec s_or f = function
	| [] -> false
	| x::xs -> if f x then true else s_or f xs
...
let is_main mn =
  let res1 = Bytes.length mn > 5 in
  if res1 then    
    begin
      let res2 = Bytes.sub mn 0 5 = "main@" in
      res2 
    end
  else
    begin
...
let flatten_path str =
  Bytes.map (fun c -> if c = '/' || c = '.' then '_' else c) str 
...
let split delim (str : bytes) : bytes list =
  if str = "" then
    []
  else
    let ls : char list = foldri (Bytes.length str) (fun i -> Bytes.get str i) in
    let to_bytes l_chars =
      (fun s c -> s ^ (Bytes.make 1 c)) |->> ("", l_chars)
    in
    let f (prevs, cur) c =
      if c = delim then
        (to_bytes (List.rev cur)::prevs, [])
      else
        (prevs, c::cur)
    in
    let (prevs, cur) = f |->> (([],[]), ls) in
    List.rev (to_bytes (List.rev cur)::prevs)
;;
