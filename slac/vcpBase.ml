...
module Shared = struct
  ...
  let rec sub_str a b =
    if Bytes.length a > 1 && Bytes.length b > 1 then
      if Bytes.get a 0 = Bytes.get b 0 then
        sub_str (Bytes.sub a 1 (Bytes.length a - 1)) (Bytes.sub b 1 (Bytes.length b - 1))
      else
        a
    else
      a
...
module VCabs = struct
  ...
  let decl_fv specifier =
    ...
    let rec dt_decl_fv = function
      ...
      | C.JUSTBASE -> []
    in
    let name_decl_fv ((_, dt, _, _), _) : bytes list =
      dt_decl_fv dt
    in
    let field_decl_fv (_, names) : bytes list =
      List.concat (name_decl_fv |>>| names)
    in
    let spec_decl_fv = function
        C.SpecType (C.Tstruct (_, Some l_fields, _)) ->
         let r : bytes list = List.concat (field_decl_fv |>>| l_fields) in
         r
      | _ -> []
    in
...
module Exp = struct
  ...
  exception VarNotFound of bytes;;
  ...
  let var_compare (x, _) (y, _) = Bytes.compare x y
  ...
  let string_to_var_attr str =
    ...
    if n < 5 then
      ...
      let c1 = Bytes.sub str (n-4) 4 in
      let init = Bytes.sub str 0 (n-4) in
    if c1 = "@hat" then
      (init, [HAT])
    ...
    | INDICES (e2) -> Bytes.concat "," (toStr |>>| e2)
    | ADDR (e) -> "&(" ^ (toStr e) ^ ")"
    | REF (e) -> "*" ^ (toStr e)
    | NEG (e) -> "!" ^ (toStr e)
  ...
  and print_attr = function
   ...
   | BAR -> "@bar"
   | EXTERN -> "<ext>"
   | FUNCPTR (rets, params) -> "(*" ^ Bytes.concat "" (print_attr |>>| rets) ^  "|" ^ Bytes.concat "" (print_attr |>>| params) ^ ")"
   | CHECK -> "@chk"
   | TILDE -> "@tilde"
   | DOT -> "@dot"
   | NESTED -> "[.]"
   ...
   | FUNC (rets, params) -> "(" ^ Bytes.concat "" (print_attr |>>| rets) ^  "|" ^ Bytes.concat "" (print_attr |>>| params) ^ ")"
  ...
  let rec get_printable_string = function
    ...
    | INDICES (e2) -> Bytes.concat "," (get_printable_string |>>| e2)
    ...
    | FCALL (fn, es) -> fn ^ "(" ^ (Bytes.concat "," (get_printable_string |>>| es)) ^ ")"
  ...
  let rec print = function
    ...
    | STRING (s) -> p "STRING "; p ("\"" ^ (Bytes.escaped s) ^ "\"")
    | ADDR (e) -> p "ADDR ("; print e; p ")"
    | NEG (e) -> p "NEG ("; print e; p ")"
    | REF (e) -> p "REF ("; print e; p ")"
    | ARROW (t, f) -> p "ARROW ("; print t; p ","; p ("\"" ^ (Bytes.escaped f) ^ "\""); p ")"
  ...
  and var_print (v, attrs) =
    let rec aux = function
      ...
      | BAR -> "BAR"
      | EXTERN -> "EXTERN"
      | FUNCPTR (rets, params) -> "FUNCPTR ([" ^ Bytes.concat "" (aux |>>| rets) ^ "],[" ^ Bytes.concat "" (aux |>>| params) ^ "])"
      | CHECK -> "CHECK"
      | TILDE -> "TILDE"
      | DOT -> "DOT"
      | NESTED -> "NESTED"
      ...
      | FUNC (rets, params) -> "FUNC ([" ^ Bytes.concat "" (aux |>>| rets) ^ "],[" ^ Bytes.concat "" (aux |>>| params) ^ "])"
    in
...
let rec var_be_typed vars to_static loc (x, _) =
  if x = "main" then
    ...
    if Bytes.contains x '#' then
      VAR (x, [EXQ])
    else if Bytes.contains x '%' then
      let i = Bytes.rindex x '%' in
      let l = Bytes.length x in
      let part = try
          Bytes.sub x (i+1) (l-i-1)
        with
  ...
  let rec __size_by_name (aliases : bytes V.t) structures s =
    if V.mem s aliases then
      let alias_name : bytes = V.find s aliases in
      let sz =
  ...
  let rec cabs_size_of (aliases : bytes V.t) structures = function
      Cabs.SpecType x::xs ->
      begin
        match x with
          Cabs.Tvoid -> CONST 8
  ...
  let rec string_to_exp_attr = function
    | VAR (v, _) ->
       ...
       let c = Bytes.get v 0 in
       if c = '&' then
         ADDR (VAR (string_to_var_attr (Bytes.sub v 1 (n-1))))
       else
  ...
  (** a + 2*a*b + b = a*(1+2*b) + b OR b*(1+2*a)+a *)
	...
	let rec toExp ((fvs, to_statics, arrays, (structures: (bytes * t list) V.t), (aliases: bytes V.t)) as packs) exp =
    match exp with
    ...
    | Cabs.CONSTANT (constant) ->
         ...
         match constant with
           ...
           let l = (Bytes.length str) - 2 in
           if l > 0 && Bytes.sub str 0 2 = "0x" then
             try
               if Bytes.length str > 20 then
                 begin
                   (* pn (Bytes.sub str 0 (l-1));
                   pn (Int64.to_string (Int64.of_string (Bytes.sub str 0 (l-1)))); 
                   (* pn (Bytes.sub str 3 (l-1)); *)
                   CONST (Scanf.sscanf (Bytes.sub str 3 (l-1)) "%x" (fun x -> x)) *)
                   NOTHING
                 end
               else
                 begin
                   (* pn (Bytes.sub str 2 l); *)
                   
                   CONST (Scanf.sscanf (Bytes.sub str 2 l) "%x" (fun x -> x))
                 end
             with
               _ -> CONST 0
           else if l > 0 && Bytes.sub str l 2 = "UL" then
             begin
               try
                 let l' = if l > 5 then 5 else l in  (** Approximation *)
                 let str' = Bytes.sub str 0 l' in
                 CONST (int_of_string str')
               with
                 _ -> CONST 1
             end
	         else
             let l = (Bytes.length str) - 1 in
             if l > 0 && Bytes.get str l = 'L' then
               begin
                 try
                   (* let l' = if l > 5 then 5 else l in  (** Approximation *) *)
                   CONST (int_of_string (Bytes.sub str 0 l))
                 with
                   _ -> CONST 1
               end
             else
    ...
    | Cabs.CALL (Cabs.VARIABLE (s), exp) when Bytes.length s > 0 && Bytes.sub s 0 1 = "@" -> 
    ...
    | FCALL (s1,t1), FCALL (s2, t2) -> let r = Bytes.compare s1 s2 in r
    | _, FCALL (_, _) -> -1
    | FCALL (_, _), _ -> 1
  	| _ -> 0
 ...
 let rec has_fsymb = function
  ...
  let rec extract_if_from_exp (pc : Cabs.expression list) (cabsloc : Cabs.cabsloc) (expr : Cabs.expression) =
    ...
    | Cabs.PAREN (x) ->
       ...
       (r, Cabs.PAREN (x'))
    | Cabs.CALL (Cabs.VARIABLE s, exps) when Bytes.length s > 0 && Bytes.sub s 0 1 = "@" ->
  ...
  and extract_if_from_bexp (pc : Cabs.expression list) (cabsloc : Cabs.cabsloc) (expr : Cabs.expression) =
    ...
    let rec bexp_to_if expr =
      ...
      | Cabs.CALL (Cabs.VARIABLE s, exps) when Bytes.length s > 0 && Bytes.sub s 0 1 = "@" ->
         ...
         (r, res)
      | Cabs.CALL (Cabs.VARIABLE s, exps) when Bytes.length s = 0->
  ...
  let rec compare x y = match x, y with
    ...
    | LBL (a, b), LBL (c, d) -> if a = c then compare b d else Bytes.compare a c
    | _ -> 0
...
module Pointer = struct
  ...
  *)
  ...
  (*
  let realize_value_from_ptr (exqs:bytes list) (b_exqs:bytes list) (var: Exp.var_t) (base_ptrs:t list) ((loc, elems):t) : Term.t list =
    let tvar = Term.encode var in
...
module Formula = struct
  type ut = string list * BExp.t list * Pointer.t list * Predicate.t list
  type t = ut list
  ...
  let (&~~) (exs, pures, ptrs, preds) pures' =
...
;;
  ...
  let rec get_real_var eq_list ptr =
    ...
    if Bytes.get sptr 0 = '.' || isnotvar then
      None
    else
      Some (sptr)
 ...
 *)
  ...
  let fpurify formula =
    let pur = (fun (a,b,c,d) ->
            ...
            let a' = List.sort_uniq (Bytes.compare) ((fun v -> v |<- (Exp.toStr |>>| fvs)) |>- a) in
            (a',b',c,d)
      ) in
    (pur |>>| formula)
