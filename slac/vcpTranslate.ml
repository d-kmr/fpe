...
module Block = struct
  ...
  type t =
    ...
    | LABEL of string * bytes list * t * Locs.t
    | BREAK of t * Locs.t
  ...
  let ptr_structs : (bytes * bytes * Exp.t list) list ref = ref []

  let structures : (bytes * (Exp.t * Term.t) list * Exp.t list option) V.t ref = ref (V.empty)
  ...
  let array_aliases : (bytes * Exp.t list) V.t ref = ref V.empty
  let s_aliases : (bytes V.t) ref = ref V.empty

  let in_func = ref "";;
  let local_statics : bytes V.t ref = ref V.empty;;

  let to_static ?local_static:(ls=false) name =
    if Bytes.contains name '%' then
      name
    else
  ...
  let get_array_dim_from_init_name is_global fvs  loc (init_name : C.init_name) specifier attrs type_name arrays_lens =
    ...
    let rec get_array_length = function
      | C.NO_INIT -> []
      | C.SINGLE_INIT (C.CONSTANT (C.CONST_STRING s)) -> [Bytes.length s + 1]
      | C.SINGLE_INIT e -> []
      | C.COMPOUND_INIT el ->
...
;;
  ...
  let rec get_var_from_exp pointers = function
    | C.VARIABLE ( str ) -> str (* if List.exists (fun (x,b) -> not b && x = str ) pointers then Bytes.cat str "[*]" else str *)
    | C.UNARY (C.MEMOF, exp) -> Bytes.cat (get_var_from_exp pointers exp) "_(0)"
    | C.UNARY (op, exp) -> Bytes.cat (t_u_op op) (get_var_from_exp pointers exp)
    | C.INDEX (exp1, exp2) ->
       Bytes.concat "_" [(get_var_from_exp pointers exp1); (get_var_from_exp pointers exp2)]
    | C.BINARY (op, exp1, exp2) ->
       get_var_from_exp pointers exp1
    (* Bytes.concat "" [(get_var_from_exp pointers exp1);(t_b_op op);(get_var_from_exp pointers exp2)] *)
    | C.CONSTANT (C.CONST_INT (str)) -> str
    | C.CONSTANT (C.CONST_FLOAT (str)) -> str
    | C.CONSTANT (C.CONST_CHAR (str)) -> Bytes.concat "" (Int64.to_string |>>| str)
    | C.CONSTANT (C.CONST_WCHAR (str)) -> Bytes.concat "" (Int64.to_string |>>| str)
    | C.CONSTANT (C.CONST_STRING (str)) -> Bytes.concat "" ["\""; str; "\""]
    | C.CONSTANT (C.CONST_WSTRING (str)) -> Bytes.concat "" (Int64.to_string |>>| str)
...
(*    | BREAK (b, p, l) ->
  ...
  let contains str sub =
    let slen = Bytes.length str in
    let blen = Bytes.length sub in
    let mlen = slen - blen in
    let rec rcont i =
      if i <= mlen then
        begin
          if Bytes.length str >= i+blen && (Bytes.sub str i blen) = sub then
            true
          else
            rcont (i+1)
        end
  ...
  let rec no_lbl = function
      C.CALL (C.VARIABLE s, _) when Bytes.length s > 1 && Bytes.sub s 0 1 = "@" -> false
  ...
  let get_declaration ?is_global:(glob=false) ?is_param:(param=false) (specifier, l_init_name) pointers cabsloc fvs  =
    ...
    and is_it_struct (fvs : Exp.attr list V.t) loc = function
      ...
      | C.SpecType (C.Tstruct (str, Some fields_l, _))::_ ->
         let str : bytes = if str = "" then new_prog_var () else str in
    ...
    and init_struct fvs (fields : Exp.t list) data =
      dbg "INIT" (Bytes.concat "," (Exp.get_printable_string |>>| fields) ^ " =") Cprint.print_init_expression data;
...
(*             | C.CAST (a, C.SINGLE_INIT exp) ->
  ...
  let rec translate (pointers : (bytes * bool) list) (fvs : (Exp.attr list) V.t) = function
    ...
    | x::xs ->
       ...
       let ((body, labels), loc) =
         match x with
         | C.LABEL (str, statement, loc) when Bytes.length str > 1 && Bytes.sub str 0 1 = "@" ->
         ...
         | C.COMPUTATION (C.BINARY (C.ASSIGN, left_exp, right_exp), loc) ->
            ...
            let is_lh_pointer = List.exists (fun (_, _, c) -> (Bytes.concat "." (Exp.var_decode |>>| c)) = left) !ptr_structs in
            if is_lh_pointer then
              let (typ, _, ns) = try List.find (fun (_, _, c) -> (Bytes.concat "." (Exp.var_decode |>>| c)) = left) !ptr_structs  with _ -> raise (StError "TR-2") in
      ...
      in *)
       ...
       | C.LABEL (str, statement, _) when Bytes.length str > 1 && Bytes.sub str 0 1 <> "@" ->
          let body' = try
               ...
               if f_is_free assigns then
                 ...
                 begin
                   ...
                   if is_malloc then
                     ...
                     | ASSIGN (v,_,_,_) ->
                        let fields =
                          if List.exists (fun (a,b,c) -> (Bytes.concat "." (__N |>>| c)) = Exp.toStr v) !ptr_structs then
                            let (typ,_,_) = try List.find (fun (a,b,c) -> (Bytes.concat "." (__N |>>| c)) = Exp.toStr v) !ptr_structs with _ -> raise (StError "TR-3") in
                            if V.mem typ !structures then
  ...
  let rec static_to_global = function
    ...
    | DECL (a, len, init_data, p, l) ->
       let p', sts = static_to_global p in
       if Bytes.get (Exp.toStr a) 0 = '^' then
...
module Structure = struct
  type t = bytes * (Exp.t * Term.t) list * Exp.t list option

  let all_struct_names : bytes list ref = ref []
  ...
  let rec get_struct pointers (structures : t V.t) ptr_struct name data (fvs : Exp.attr list V.t) loc : t * (bytes * bytes * Exp.t list) list * t list * Block.t list * Exp.t list =
...
module Procedure = struct
  ...
  let translate pointers structures fvs  ((spec, (s_name, dt, xx, l)), (block:C.block)) cabsloc fpdata : (t * Block.t list) =
    ...
    (* let block = VcpFpOnC.fp_transform_function fpdata s_name structures block' in *)
...
module Global = struct
  ...
  let unfold_struct sofar_structures pointers ptr_struct loc fvs specs =
    ...
    | (y::ys) ->
       ...
       | C.SpecType (C.Tstruct (str, o_l_fields, _)) ->
          ...
          let (current_structs, p_s, aux_structs, aux_blocks, fvs') : Structure.t * (bytes * bytes * Exp.t list) list * Structure.t list * Block.t list * Exp.t list =
  ...
  let rec translate ?(dummy=false) acc pointers (sofar_structures : Structure.t V.t) (ptr_struct : (bytes * bytes * Exp.t list) list) procs struct_map fvs fpdata = function
    ...
    | x::_xs ->
       ...
       | C.FUNDEF ((spec, name), block, something, cabsloc) ->
       ...
       | (C.DECDEF ((l_spec_elem, l_init_name), cabsloc)) as dec ->
          ...
          begin
            ...
            let old_refs : bytes list = all_olds g_name in (* fst |>>| (List.filter (fun (_, old_s) -> old_s = g_name) struct_map) in *)
